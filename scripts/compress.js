 ...
 */
{
    ...
    for (const inputFile of files) {
        ...
        const command = spawn('gltf-transform', ['etc1s', inputFile, ktx2File, '--quality', '255', '--verbose'])
        ...
        command.on('close', (code) => {
            ...
            dracoCommand.stdout.on('data', (data) => {
                console.log(`stdout: ${data}`)
            })
            dracoCommand.stderr.on('data', (data) => {
                console.error(`stderr: ${data}`)
            })
        })
    }
 ...
 */
{
    ...
    const presets = [
        ...
        [
            /whispers\/whisperFlame.png$/,
            '--nowarn --2d --t2 --encode uastc --qlevel 255 --assign_oetf linear --target_type R --swizzle r001'
        ],
        [
            /achievements\/glyphs.png$/,
            '--nowarn --2d --t2 --encode etc1s --qlevel 255 --assign_oetf linear --target_type R --swizzle r001'
        ],
        [
            /areas\/satanStar.png$/,
        ...
        [
            /foliage\/foliageSDF.png$/,
            '--nowarn --2d --t2 --encode etc1s --qlevel 255 --assign_oetf linear --target_type R --swizzle r001'
        ],
        [
            /interactivePoints\/.+.png$/,
            '--nowarn --2d --t2 --encode etc1s --qlevel 255 --assign_oetf linear --target_type R --swizzle r001'
        ],
        [
            /intro\/.+.png$/,
        ...
        [
            /terrain\/terrain.png$/,
            '--nowarn --2d --t2 --encode uastc --genmipmap --assign_oetf linear --target_type RGB'
        ],
        [/career\/.+png$/, '--nowarn --2d --t2 --encode uastc --assign_oetf srgb --target_type RG'],
        [
            /whispers\/whisperFlame.png$/,
            '--nowarn --2d --t2 --encode etc1s --qlevel 255 --assign_oetf linear --target_type R'
        ]
    ]
    ...
    for (const inputFile of files) {
        ...
        const command = spawn('toktx', [...preset.split(' '), ktx2File, inputFile])
