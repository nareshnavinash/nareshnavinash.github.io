...
async function main() {
    ...
    const statuesToRemove = [
        'blueskyPhysicalDynamic',
        'youtubePhysicalDynamic',
        'twitchPhysicalDynamic',
        'discordPhysicalDynamic',
        'mediumPhysicalDynamic',
        'npmPhysicalDynamic',
        'pypiPhysicalDynamic',
        'contactPhysicalDynamic'
    ]
    ...
    const origLast = [-10.87, -2.528, 5.725] // LEFT side
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (landingNode) {
            ...
            console.log(
                `  Added letter: ${node.getName()} local=(${lt.map((v) => v.toFixed(2)).join(', ')}) [+cuboid collider]`
            )
        }
    }
    ...
    for (const logoNode of logosScene.listChildren()) {
        ...
        if (socialNode) {
            ...
            console.log(
                `  Added logo: ${newNode.getName()} local=(${lt.map((v) => v.toFixed(2)).join(', ')}) [+cuboid collider]`
            )
        }
    }
