...
export default class Foliage {
  ...
  _createBushes() {
    ...
    const areaCenters = Object.values(areas).map((a) => ({
      x: a.position.x, z: a.position.z, r: a.radius + 2,
    }))
    ...
    while (placed < BUSH_COUNT && attempts < BUSH_COUNT * 4) {
      ...
      for (const a of areaCenters) {
        if ((x - a.x) ** 2 + (z - a.z) ** 2 < a.r * a.r) { skip = true; break }
      }
      if (skip) continue
  ...
  _createFlowers() {
    ...
    const areaCenters = Object.values(areas).map((a) => ({
      x: a.position.x, z: a.position.z, r: a.radius,
    }))
    ...
    while (placed < FLOWER_COUNT && attempts < FLOWER_COUNT * 4) {
      ...
      let skip = false
      for (const a of areaCenters) {
        if ((x - a.x) ** 2 + (z - a.z) ** 2 < (a.r * 0.5) ** 2) { skip = true; break }
      }
      if (skip) continue
