...
export class VisualVehicle {
    ...
    setParts() {
        ...
        // Blinker left
        if (this.parts.blinkerLeft) this.parts.blinkerLeft.visible = false

        // Blinker right
        if (this.parts.blinkerRight) this.parts.blinkerRight.visible = false

        // Stop lights
        if (this.parts.stopLights) this.parts.stopLights.visible = false
    ...
    setPaints() {
        ...
        this.paints.choices.orange = this.game.materials.createGradient(
            'orangeGradient',
            '#ff940d',
            '#af0071',
            this.game.materials.debugPanel?.addFolder({ title: 'orangeGradient' })
        )
        this.paints.choices.white = this.game.materials.createGradient(
            'whiteGradient',
            '#ffffff',
            '#b5b5b5',
            this.game.materials.debugPanel?.addFolder({ title: 'whiteGradient' })
        )
        this.paints.choices.black = this.game.materials.createGradient(
            'blackGradient',
            '#626262',
            '#262526',
            this.game.materials.debugPanel?.addFolder({ title: 'blackGradient' })
        )
        ...
        // Flames
        {
            ...
            material.outputNode = Fn(() => {
                // Flames
                {
                    ...
                    const uv3 = newUv
                        .sub(vec2(0, this.game.ticker.elapsedScaledUniform.mul(-0.075)))
                        .mul(vec2(0.96 * 1.3, 0.35 * 1.3))
                    const noise3 = texture(this.game.noises.voronoi, uv3).r

                    const uv4 = newUv
                        .sub(vec2(0, this.game.ticker.elapsedScaledUniform.mul(-0.041)))
                        .mul(vec2(1.28 * 1.3, 0.75 * 1.3))
            ...
            if (this.game.debug.active && this.game.materials.debugPanel) {
                ...
                debugPanel.addBinding(emissiveStrength, 'value', {
        ...
        // Abyssal
        {
            ...
            material.outputNode = Fn(() => {
                ...
                const starsColor = texture(this.game.resources.behindTheSceneStarsTexture, starsUv)
                    .rgb.pow(2)
                    .mul(starsIntensity)
            ...
            if (this.game.debug.active && this.game.materials.debugPanel) {
                ...
                debugPanel.addBinding(fresnelIntensity, 'value', {
                    label: 'fresnelIntensity',
                    min: 1,
                    max: 40,
                    step: 0.001
                })
                debugPanel.addBinding(starsIntensity, 'value', {
                    label: 'starsIntensity',
                    min: 1,
                    max: 40,
                    step: 0.001
                })
        ...
        this.paints.changeTo = (name = 'red') => {
            ...
            for (const wheel of this.wheels.items) {
                if (wheel.painted) wheel.painted.material = material
            }
        }
    ...
    setWheels() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            wheel.container.traverse((child) => {
                if (child.name.match(/^wheelSuspension/)) wheel.suspension = child
                if (child.name.match(/^wheelCylinder/)) wheel.cylinder = child
                if (child.name.match(/^wheelPainted/)) wheel.painted = child
            })

            // Cylinder (actual wheel)
            wheel.cylinder.position.set(0, 0, 0)

            if (i === 0 || i === 2) wheel.container.rotation.y = Math.PI
    ...
    setBlinkers() {
        if (!this.parts.blinkerLeft) return
    ...
    update() {
        ...
        // Wheels
        this.wheels.steering +=
            (this.game.player.steering * physicalVehicle.steeringAmplitude - this.wheels.steering) *
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (
                !this.game.inputs.actions.get('brake').active ||
                this.game.inputs.actions.get('forward').active ||
                this.game.inputs.actions.get('backward').active
            ) {
                if (i === 0 || i === 2) visualWheel.cylinder.rotation.z += wheelsRotation
                else visualWheel.cylinder.rotation.z -= wheelsRotation
            }

            if (i === 0) visualWheel.container.rotation.y = Math.PI + this.wheels.steering

            if (i === 1) visualWheel.container.rotation.y = this.wheels.steering
        ...
        if (this.game.player.braking) {
            if (this.parts.stopLights) this.parts.stopLights.visible = true
        ...
        // Boost trails
        const trailAlpha =
            physicalVehicle.goingForward && this.game.player.boosting && this.game.player.accelerating > 0 ? 1 : 0
        this.boostTrails.leftReference.getWorldPosition(this.boostTrails.left.position)
        ...
        // Boost animation
        this.boostAnimation.mix +=
            (this.game.player.boosting ? 1 : -1) * this.game.ticker.deltaScaled * this.boostAnimation.speed
