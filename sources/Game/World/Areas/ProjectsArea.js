...
export class ProjectsArea extends Area {
    ...
    setInputs() {
        ...
        window.addEventListener('keydown', (_event) => {
            if (
                _event.code === 'Escape' &&
                (this.state === ProjectsArea.STATE_OPEN || this.state === ProjectsArea.STATE_OPENING)
            )
                this.close()
        })
        ...
        this.game.inputs.interactiveButtons.events.on('previous', () => {
            if (this.state === ProjectsArea.STATE_OPEN) this.previous()
        })

        this.game.inputs.interactiveButtons.events.on('next', () => {
            if (this.state === ProjectsArea.STATE_OPEN) this.next()
        })

        this.game.inputs.interactiveButtons.events.on('open', () => {
            if (this.state === ProjectsArea.STATE_OPEN) this.url.open()
        })

        this.game.inputs.interactiveButtons.events.on('close', () => {
            if (this.state === ProjectsArea.STATE_OPEN) this.close()
        })
    }
    ...
    setCinematic() {
        ...
        this.cinematic.targetOffset = new THREE.Vector3(-3.0, 1.6, -4.6)
    ...
    setImages() {
        ...
        this.images.loadSibling = () => {
            ...
            if (this.images.direction === ProjectsArea.DIRECTION_PREVIOUS) imageIndex -= 1
            else imageIndex += 1
            ...
            if (imageIndex < 0) {
                ...
                if (projectIndex > projectsData.length - 1) projectIndex = 0
        ...
        this.images.getResourceAndLoad = (key) => {
            ...
            if (!resource) {
                ...
                const pageContent =
                    project && project.pages ? project.pages[pageIndex] : { header: key, subheader: '', points: [] }
    ...
    setPagination() {
        ...
        this.pagination.update = () => {
            ...
            for (const item of this.pagination.items) {
                if (i <= this.navigation.current.images.length - 1) {
                    if (!item.visible) {
                        ...
                        item.visible = true
                        item.intersect.active =
                            this.state === ProjectsArea.STATE_OPENING || this.state === ProjectsArea.STATE_OPEN
                    }
                } else {
                    if (item.visible) {
                        gsap.to(item.mesh.scale, {
            ...
            gsap.to(this.pagination.group.position, {
                x: offset,
                duration: 0.5,
                ease: 'power1.inOut',
                overwrite: true,
    ...
    setAttributes() {
        ...
        this.attributes.wrapText = (textCanvas, value) => {
            ...
            for (const line of lines) {
                ...
                for (const word of words) {
                    ...
                    if (textCanvas.context.measureText(test).width > maxWidth && current) {
                        ...
                        if (wrapped.length >= this.attributes.maxLines) break
                    } else {
                        current = test
                    }
                }
            ...
            if (wrapped.length > this.attributes.maxLines) wrapped.length = this.attributes.maxLines
        ...
        this.attributes.getRoleForImage = (imageIndex = this.images.index) => {
            ...
            if (role instanceof Array) return role[0]

            return role
        }

        this.attributes.getValue = (name, imageIndex = this.images.index) => {
            if (name === 'role') return this.attributes.getRoleForImage(imageIndex)
    ...
    setAdjacents() {
        ...
        // Text
        this.adjacents.previous.textMesh = this.adjacents.previous.inner.children.find((_child) =>
            _child.name.startsWith('text')
        )
        ...
        // Text
        this.adjacents.next.textMesh = this.adjacents.next.inner.children.find((_child) =>
            _child.name.startsWith('text')
        )
        ...
        this.adjacents.update = () => {
            if (this.adjacents.status === 'hiding') return

            this.adjacents.status = 'hiding'

            gsap.to(this.adjacents.previous.inner.rotation, {
                z: Math.PI * 0.5,
                duration: 0.5,
                delay: 0,
                ease: 'power2.in',
            ...
            gsap.to(this.adjacents.next.inner.rotation, {
                z: -Math.PI * 0.5,
                duration: 0.5,
                delay: 0.2,
                ease: 'power2.in',
    ...
    setDistinctions() {
        ...
        this.distinctions.items.awwwards = this.distinctions.group.children.find((_child) =>
            _child.name.startsWith('awwwards')
        )
        this.distinctions.items.fwa = this.distinctions.group.children.find((_child) => _child.name.startsWith('fwa'))
        this.distinctions.items.cssda = this.distinctions.group.children.find((_child) =>
            _child.name.startsWith('cssda')
        )
        ...
        this.distinctions.update = () => {
            if (this.distinctions.status === 'hiding') return
    ...
    setOven() {
        ...
        const alphaNode = Fn(() => {
            ...
            const voronoi = texture(this.game.noises.voronoi, baseUv).g
    ...
    open() {
        if (this.state === ProjectsArea.STATE_OPEN || this.state === ProjectsArea.STATE_OPENING) return
        ...
        // Cursor
        for (const item of this.pagination.items) item.intersect.active = item.visible
    ...
    close() {
        if (this.state === ProjectsArea.STATE_CLOSED || this.state === ProjectsArea.STATE_CLOSING) return
        ...
        // Cursor
        for (const item of this.pagination.items) item.intersect.active = false
    ...
    previous() {
        if (this.images.index > 0) this.previousImage()
        else this.previousProject(false)
    }
    ...
    next() {
        if (this.images.index < this.navigation.current.images.length - 1) this.nextImage()
    ...
    changeProject(index = 0, direction = ProjectsArea.DIRECTION_NEXT, firstImage = false, silent = false) {
        ...
        if (loopIndex > projectsData.length - 1) loopIndex = 0
        else if (loopIndex < 0) loopIndex = projectsData.length - 1

        // Save
        this.navigation.index = loopIndex
        this.navigation.current = projectsData[this.navigation.index]
        this.navigation.previous =
            projectsData[this.navigation.index - 1 < 0 ? projectsData.length - 1 : this.navigation.index - 1]
        this.navigation.next = projectsData[(this.navigation.index + 1) % projectsData.length]

        // Change image
        let imageIndex = null
        if (firstImage) imageIndex = 0
        else imageIndex = direction === ProjectsArea.DIRECTION_NEXT ? 0 : this.navigation.current.images.length - 1
    ...
    update() {
        ...
        if (loopTime < this.anvil.loopTime) this.sounds.anvil.play()
