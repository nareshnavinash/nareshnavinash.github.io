        ...
        // Still color and foam
        {
            const colorNode = Fn(() => {
                ...
                // Foam
                {
                    ...
                    const uv3 = newUv
                        .sub(vec2(this.game.ticker.elapsedScaledUniform.mul(0.05), 0))
                        .mul(vec2(0.35, 0.96))
                    const noise3 = texture(this.game.noises.voronoi, uv3).r

                    const uv4 = newUv
                        .sub(vec2(this.game.ticker.elapsedScaledUniform.mul(0.041), 0))
                        .mul(vec2(0.75, 1.28))
        ...
        // Drop foam
        {
            const colorNode = Fn(() => {
                ...
                // Foam
                {
                    ...
                    const uv4 = baseUv
                        .sub(vec2(0, this.game.ticker.elapsedScaledUniform.mul(0.085)))
                        .mul(vec2(1.5, 0.8))
                    const noise4 = texture(this.game.noises.voronoi, uv4).r
        ...
        // Particles
        {
            ...
            material.positionNode = Fn(() => {
                const progress = this.game.ticker.elapsedScaledUniform
                    .mul(0.2)
                    .add(float(instanceIndex).div(count))
                    .fract()
