...
export class Foliage {
    ...
    setGeometry() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // Position
            const spherical = new THREE.Spherical(1 - Math.pow(rng(), 3), Math.PI * 2 * rng(), Math.PI * rng())
    ...
    setMaterial() {
        ...
        const foliageAlpha = Fn(() => {
            const rotatedUv = rotateUV(uv(), this.game.wind.offsetNode(positionLocal.xz).length().mul(2.2), vec2(0.5))
        ...
        const alphaNode = Fn(() => {
            ...
            if (this.seeThrough) {
                ...
                // Alpha
                alpha.assign(
                    foliageSDF.mul(distanceFade.mul(this.material.threshold.oneMinus()).add(this.material.threshold))
                )
            } else {
                // Alpha
        ...
        this.material.instance.receivedShadowPositionNode = positionLocal.add(
            this.game.lighting.directionUniform.mul(this.material.shadowOffset)
        )
    ...
    update() {
        ...
        this.material.seeThroughEdgeMin.value =
            (3 / this.game.view.spherical.radius.current) * this.seeThroughMultiplier
        this.material.seeThroughEdgeMax.value =
            (15 / this.game.view.spherical.radius.current) * this.seeThroughMultiplier
    }
}
