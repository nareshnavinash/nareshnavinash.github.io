...
export class VisualTornado {
    ...
    setMesh() {
        ...
        material.positionNode = twistedCylinder(
            positionLocal,
            parabolStrength,
            parabolOffset,
            parabolAmplitude.sub(0.05),
            this.game.ticker.elapsedScaledUniform.mul(timeScale).mul(2)
        )
        ...
        material.outputNode = Fn(() => {
             ...
             */
            // Noise 1
            const gooNoise1Uv = uv()
                .add(vec2(scaledTime.mul(0.88), scaledTime.mul(0.88)))
                .add(vec2(0.5))
            ...
            // Noise 2
            const gooNoise2Uv = uv()
                .add(vec2(scaledTime.mul(0.66), scaledTime.mul(0.66)))
                .add(vec2(0.5))
        ...
        if (this.game.debug.active) {
            ...
            this.debugPanel.addBinding(parabolStrength, 'value', {
                label: 'parabolStrength',
                min: 0,
                max: 4,
                step: 0.001
            })
            this.debugPanel.addBinding(parabolOffset, 'value', { label: 'parabolOffset', min: 0, max: 4, step: 0.001 })
            this.debugPanel.addBinding(parabolAmplitude, 'value', {
                label: 'parabolAmplitude',
                min: 0,
                max: 4,
                step: 0.001
            })
