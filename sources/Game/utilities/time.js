...
export function timeToReadableString(time, withHours = true, widthMinutes = true, withSeconds = true) {
    ...
    if (hours > 0 && withHours) parts.push(`${hours}h`)

    if ((hours > 0 || minutes > 0) && widthMinutes) parts.push(`${minutes}m`)

    if ((hours > 0 || minutes > 0 || seconds > 0) && withSeconds) parts.push(`${seconds}s`)
