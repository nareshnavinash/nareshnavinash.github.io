        ...
        this.outputNode = Fn(() => {
            ...
            if (this.hasLightBounce) {
                const bounceOrientation = reorientedNormal
                    .dot(vec3(0, -1, 0))
                    .smoothstep(this.game.lighting.lightBounceEdgeLow, this.game.lighting.lightBounceEdgeHigh)
                const bounceDistance = this.game.lighting.lightBounceDistance
                    .sub(max(0, positionWorld.y))
                    .div(this.game.lighting.lightBounceDistance)
                    .max(0)
                    .pow(2)
                ...
                outputColor.assign(
                    mix(
                        outputColor,
                        bounceColor,
                        bounceOrientation.mul(bounceDistance).mul(this.game.lighting.lightBounceMultiplier)
                    )
                )
            }
            ...
            if (this.hasWater) {
                const nearWaterSurface = positionWorld.y
                    .sub(this.game.water.surfaceElevationUniform)
                    .abs()
                    .greaterThan(this.game.water.surfaceThicknessUniform)
            ...
            if (this.hasCoreShadows)
                coreShadowMix = reorientedNormal
                    ...
                    .smoothstep(this.game.lighting.coreShadowEdgeHigh, this.game.lighting.coreShadowEdgeLow)
            ...
            // Fog
            if (this.hasFog) outputColor.assign(this.game.fog.strength.mix(outputColor, this.game.fog.color))

            // Alpha test discard
            this._alphaNode.lessThan(this.alphaTest).discard()

            // Reveal
            if (this.hasReveal) outputColor.assign(MeshDefaultMaterial.revealDiscardNodeBuilder(this.game, outputColor))
