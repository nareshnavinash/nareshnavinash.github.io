...
export class Objects {
    ...
    add(_visualDescription = null, _physicalDescription = null) {
        ...
        if (_visualDescription && _visualDescription.model) {
            ...
            // Update materials
            if (visualDescription.updateMaterials) this.game.materials.updateObject(visualDescription.model)

            // Update shadows
            if (visualDescription.castShadow || visualDescription.receiveShadow) {
                visualDescription.model.traverse((_child) => {
                    if (_child.isMesh) {
                        if (visualDescription.castShadow) _child.castShadow = true

                        if (visualDescription.receiveShadow) _child.receiveShadow = true
                    }
                })
            }

            // Add to scene
            if (visualDescription.parent !== null) visualDescription.parent.add(visual.object3D)
    ...
    getFromModel(_model, _visualDescription = {}, _physicalDescription = {}) {
        ...
        if (physical) {
            ...
            if (typeof _physicalDescription.type === 'undefined') {
                ...
                if (_model.name.match(/dynamic/i)) _physicalDescription.type = 'dynamic'
            ...
            for (const _child of children) {
                ...
                if (typeof _child.userData.friction !== 'undefined') collider.friction = _child.userData.friction

                if (typeof _child.userData.category !== 'undefined') collider.category = _child.userData.category
    ...
    update() {
        ...
        this.list.forEach((_object) => {
            ...
            if (
                _object.visual &&
                _object.physical &&
                (_object.needsUpdate || (!_object.physical.body.isSleeping() && _object.physical.body.isEnabled()))
            ) {
