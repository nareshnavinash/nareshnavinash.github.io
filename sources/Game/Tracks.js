...
export class Tracks {
    constructor() {
        ...
        this.renderTarget = new THREE.RenderTarget(this.resolution, this.resolution, {
...
export class Track {
    ...
    setTrail() {
        ...
        this.trail.material = new THREE.MeshBasicNodeMaterial({
            wireframe: false,
            depthTest: false,
            transparent: true,
        ...
        this.trail.material.positionNode = Fn(() => {
            ...
            const trackUVPrev = vec2(ratio.sub(fragmentSize), 0.5)
            trackData.assign(texture(this.dataTexture, trackUV))
            const trackDataPrev = texture(this.dataTexture, trackUVPrev)

            const angle = atan(trackData.z.sub(trackDataPrev.z), trackData.x.sub(trackDataPrev.x))
            ...
            const newPosition = vec3(trackData.x.add(trailPosition.x), trackData.y, trackData.z.add(trailPosition.y))
