...
export class View {
    ...
    constructor(idealRatio = 1920 / 1080) {
        ...
        // Toggle
        if (this.game.debug.active) {
            this.game.inputs.addActions([{ name: 'viewToggle', categories: [], keys: ['Keyboard.KeyV'] }])
    ...
    setFocusPoint() {
        ...
        this.focusPoint.helper = new THREE.Mesh(
            new THREE.SphereGeometry(0.5, 32, 32),
            new THREE.MeshBasicNodeMaterial({ color: new THREE.Color('orange').multiplyScalar(4), wireframe: false })
        )
    ...
    setOptimalArea() {
        ...
        this.optimalArea.update = () => {
            ...
            let radiusMax =
                this.spherical.radius.edges.max + this.ratioOverflow * this.spherical.radius.nonIdealRatioOffset
    ...
    setRoll() {
        ...
        if (this.game.debug.active) {
            ...
            rollDebugPanel.addButton({ title: 'kick' }).on('click', () => {
                this.roll.kick()
            })
    ...
    setCinematic() {
        ...
        this.cinematic.start = (position, target) => {
            ...
            if (this.ratioOverflow > 0) {
                const delta = this.cinematic.position
                    .clone()
            ...
            gsap.to(this.game.rendering.cheapDOFPass.strength, {
                value: 0,
                duration: 1.5,
                ease: 'power2.inOut',
                overwrite: true
    ...
    setSpeedLines() {
        ...
        this.speedLines.geometry.setAttribute(
            'timeRandomness',
            new THREE.Float32BufferAttribute(timeRandomnessArray, 1)
        )
        ...
        this.speedLines.material = new THREE.MeshBasicNodeMaterial({
            wireframe: false,
            depthWrite: false,
            depthTest: false
        })
        this.speedLines.material.vertexNode = Fn(() => {
            ...
            const osciliation = this.game.ticker.elapsedScaledUniform
            ...
            const newPosition = mix(
                ...
                tipness.mul(osciliation).mul(distance).mul(this.speedLines.smoothedStrength)
            )
        ...
        this.game.inputs.events.on('viewMapPointer', (action) => {
            if (this.mode === View.MODE_DEFAULT) {
                ...
                if (action.active) {
                    // Map
                    if (
                        this.game.inputs.pointer.mode === Pointer.MODE_MOUSE ||
                        this.game.inputs.pointer.touches.length >= 2
                    ) {
                        this.focusPoint.isTracking = false

                        const mapMovement = new THREE.Vector2(
                            this.game.inputs.pointer.delta.x,
                            this.game.inputs.pointer.delta.y
                        )
    ...
    update() {
        ...
        if (
            this.mode === View.MODE_DEFAULT &&
            this.game.inputs.gamepad.joysticks.right.active &&
            !this.cinematic.active
        ) {
            this.focusPoint.isTracking = false

            const mapMovement = new THREE.Vector2(
                this.game.inputs.gamepad.joysticks.right.x,
                this.game.inputs.gamepad.joysticks.right.y
            )
            mapMovement.rotateAround(new THREE.Vector2(), -this.spherical.theta)
        ...
        if (this.focusPoint.helper.visible) this.focusPoint.helper.position.copy(newSmoothFocusPoint)
        ...
        if (this.optimalArea.quad2Helper && this.optimalArea.quad2Helper.visible) {
            ...
            geometry.setPositions([
                ...
                this.optimalArea.quad2[1].offseted.x,
                0.5,
                this.optimalArea.quad2[1].offseted.y,
                this.optimalArea.quad2[2].offseted.x,
                0.5,
                this.optimalArea.quad2[2].offseted.y,
                this.optimalArea.quad2[3].offseted.x,
                0.5,
                this.optimalArea.quad2[3].offseted.y,
                this.optimalArea.quad2[0].offseted.x,
                0.5,
                this.optimalArea.quad2[0].offseted.y
            ])
        ...
        this.speedLines.smoothedStrength.value = lerp(
            this.speedLines.smoothedStrength.value,
            this.speedLines.strength,
            this.game.ticker.delta * 2
        )
    }
}
