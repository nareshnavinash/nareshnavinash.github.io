...
export class Inputs {
    ...
    setGamepad() {
        ...
        this.gamepad.events.on('joystickChange', (joystick) => {
            this.change(`Gamepad.joystick${joystick.name.charAt(0).toUpperCase() + joystick.name.slice(1)}`, {
                x: joystick.x,
                y: joystick.y,
    ...
    setNipple() {
        ...
        this.addActions([{ name: 'nipplePointer', categories: ['wandering', 'racing'], keys: ['Pointer.any'] }])
