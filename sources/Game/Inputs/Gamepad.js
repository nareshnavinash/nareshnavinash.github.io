...
export class Gamepad {
    ...
    setMappings() {
        ...
        this.mappings.list.windowsFirefoxPS5 = [
            ...
            { name: 'up', type: 'button', action: 'axisToArrow', index: 9, angles: [-1, -0.7142857142857143, 1] },
            {
                ...
                action: 'axisToArrow',
                index: 9,
                angles: [-0.1428571428571429, 0.1428571428571428, 0.4285714285714286]
            },
            {
                name: 'left',
                type: 'button',
                action: 'axisToArrow',
                index: 9,
                angles: [0.4285714285714286, 0.7142857142857142, 1]
            },
            {
                name: 'right',
                type: 'button',
                action: 'axisToArrow',
                index: 9,
                angles: [-0.7142857142857143, -0.4285714285714286, -0.1428571428571429]
            },
        ...
        this.mappings.getNameForGamepad = (gamepad) => {
            ...
            if (gamepad.axes.length === 10) return 'windowsFirefoxPS5'
            else if (gamepad.axes.length === 7) return 'macOSFirefoxPS5'
            ...
            for (const map of mapping) {
                ...
                if (map.type === 'button') {
                    ...
                    if (map.action === 'buttonRaw') {
                        ...
                        if (mapButton) button.value = mapButton.value
                    } else if (map.action === 'axisToTrigger') {
                        const mapAxes = gamepad.axes[map.index]

                        if (mapAxes) button.value = mapAxes * 0.5 + 0.5
                        ...
                        if (mapAxes) {
                            ...
                            for (const angle of map.angles) {
                                if (Math.abs(angle - arrowAngle) < 0.1) isInAngles = true
                            }

                            if (isInAngles) button.value = 1
                        }
                    }
                ...
                if (map.type === 'joystick') {
                    ...
                    if (map.action === 'axesToCircle') {
                        ...
                        joystick.safeX =
                            remapClamp(Math.abs(joystick.x), joystickDeadZone, 1, 0, 1) * Math.sign(joystick.x)

                        joystick.y = gamepad.axes[map.indexes[1]]
                        joystick.safeY =
                            remapClamp(Math.abs(joystick.y), joystickDeadZone, 1, 0, 1) * Math.sign(joystick.y)
    ...
    setButtons() {
        ...
        for (const map of this.mappings.list.standard) {
            if (map.type === 'button') this.buttons[map.name] = { name: map.name, value: 0, pressed: 0 }
        }
    }
    ...
    update() {
        ...
        for (const _gamepad of navigator.getGamepads()) {
            if (_gamepad !== null) gamepad = _gamepad
        }
        ...
        for (const buttonName in this.buttons) {
            ...
            if (newButton.pressed) {
                if (!oldPressed) this.events.trigger('down', [savedButton])
            } else {
                if (oldPressed) this.events.trigger('up', [savedButton])
            }

            if (newButton.value !== oldValue) this.events.trigger('change', [savedButton])
        }
