...
export class Nipple {
    ...
    setMeshes() {
        ...
        material.outputNode = Fn(() => {
            ...
            // Angle
            const directionAngleSDF = abs(radialAngle)
                .sub(this.forwardAmplitude * 0.5)
                .toVar()
            ...
            const outerEdgeFill = step(outerEdgeSDF, this.edgesThickness / 2)
                .mul(directionAngle)
                .toVar()
            ...
            const progressSDF = radialCoord
                ...
                .toVar() // Distance between edges minus outlines
        ...
        if (action.trigger === 'start' || action.trigger === 'change') {
            if (this.active) {
                ...
                if (pointer.touches.length == 1) {
                    ...
                    // Progress
                    this.progress = clamp(
                        (distance - this.progressRadiusLow) / (this.progressRadiusHigh - this.progressRadiusLow),
                        0,
                        1
                    )
