...
export class Modals {
    ...
    setItems() {
        ...
        for (const element of elements) {
            ...
            if (typeof element.dataset.default !== 'undefined') this.default = item
        }
    }
        ...
        // Currently closed => Open immediately
            ...
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    ...
                    // Tabs resize
                    if (item.tabs) item.tabs.resize()

                    // Focus
                    if (item.mainFocus) item.mainFocus.focus()
                })
            })
    ...
    close() {
        if (this.state === Modals.CLOSING || this.state === Modals.CLOSED) return
