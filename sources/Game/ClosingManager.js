    ...
    constructor() {
        ...
        this.game.inputs.addActions([
            {
                ...
                keys: ['Keyboard.Escape', 'Gamepad.cross']
            },
            {
                name: 'pause',
                categories: ['modal', 'menu', 'racing', 'cinematic', 'wandering'],
                keys: ['Gamepad.start']
            }
        ])
        ...
        this.game.inputs.events.on('close', (action) => {
            if (action.active) {
                ...
                if (this.game.world.whispers?.menu.inputFlag.isOpen) this.game.world.whispers.menu.inputFlag.close()
                ...
                // Modal open => Close
                else if (this.game.modals.state === Modals.OPEN) this.game.modals.close()
                // Menu open => Close
                else if (
                    this.game.menu.state === Menu.OPEN ||
                    (this.game.inputs.mode !== Inputs.MODE_GAMEPAD && this.game.menu.state === Menu.OPENING)
                )
                ...
                // Projects => Close
                    ...
                    this.game.world.areas?.projects &&
                    (this.game.world.areas?.projects.state === ProjectsArea.STATE_OPEN ||
                        this.game.world.areas?.projects.state === ProjectsArea.STATE_OPENING)
                )
                    this.game.world.areas.projects.close()
                // Lab => Close
                else if (
                    this.game.world.areas?.lab &&
                    (this.game.world.areas?.lab.state === LabArea.STATE_OPEN ||
                        this.game.world.areas?.lab.state === LabArea.STATE_OPENING)
                )
        ...
        this.game.modals.events.on('open', () => {
            if (this.game.menu.state === Menu.OPEN || this.game.menu.state === Menu.OPENING) this.game.menu.close()
        })
        ...
        // On modal close => Restore correct input filter
