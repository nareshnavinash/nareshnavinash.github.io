...
export class Notifications {
    ...
    show(html = '', type = '', duration = 3, clickCallback = null, id = null) {
        ...
        if (type) item.element.classList.add(`is-${type}`)
        ...
        item.element.addEventListener('pointerenter', (event) => {
            if (event.pointerType !== 'touch') this.timeHold = true
        })
    ...
    hide() {
        if (this.state === Notifications.STATE_HIDING || this.state === Notifications.STATE_HIDDEN) return
