...
export class Audio {
    ...
    register(options = {}) {
        ...
        if (item.positions !== null && !(item.positions instanceof Array)) item.positions = [item.positions]
        ...
        item.play = (...parameters) => {
            ...
            // Anti spam
            if (item.antiSpam) {
                if (this.game.ticker.elapsed - item.lastPlay < item.antiSpam) return
            }

            // Play binding
            if (typeof item.onPlay === 'function') item.onPlay(item, ...parameters)
    ...
    setPlaylist() {
        ...
        this.playlist.next = () => {
            if (this.playlist.switching) return
            ...
            gsap.delayedCall(3, () => {
                ...
                if (this.playlist.index >= this.playlist.songs.length) this.playlist.index = 0
    ...
    setMute() {
        ...
        this.mute.toggle = () => {
            if (this.mute.active) this.mute.deactivate()
            else this.mute.activate()
        }
        ...
        if (soundToggleLocal !== null && soundToggleLocal === '1') this.mute.activate()
        ...
        this.game.inputs.events.on('mute', (action) => {
            if (action.active) this.mute.toggle()
        })
        ...
        window.addEventListener('blur', () => {
            ...
            if (this.playlist?.current) this.playlist.current.sound.pause()
        })
        ...
        window.addEventListener('focus', () => {
            if (!this.mute.active) {
                ...
                if (this.playlist?.current) this.playlist.current.sound.play()
            }
        })
    }
