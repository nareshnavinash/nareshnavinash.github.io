...
export class RayCursor {
    ...
    addIntersect(description) {
        ...
        } else intersect.position = intersect.shape.center
    ...
    testIntersects(actionTrigger) {
        ...
        if (actionTrigger === 'start' || actionTrigger === 'change') {
            ...
            const intersects = this.intersects.filter((intersect) => {
                ...
                if (distance > this.game.view.optimalArea.radius) return false
            ...
            if (intersects.length) {
                ...
                for (const intersect of intersects) {
                    ...
                    if (isIntersecting !== intersect.isIntersecting) {
                        ...
                        if (intersect.isIntersecting) {
                            ...
                            if (typeof intersect.onEnter === 'function') intersect.onEnter()
                        } else {
                            if (typeof intersect.onLeave === 'function') intersect.onLeave()
                        }
                    }
                ...
                if (isAnyIntersecting !== this.isAnyIntersecting) {
                    ...
                    if (this.isAnyIntersecting) this.game.canvasElement.style.cursor = 'pointer'
                    else this.game.canvasElement.style.cursor = null
                }
            ...
            if (actionTrigger === 'start') {
                if (this.currentIntersect) {
                    ...
                    if (typeof this.currentIntersect.onDown === 'function') this.currentIntersect.onDown()
                }
            }
        }
            ...
            for (const intersect of intersects) {
                if (intersect.isIntersecting) {
                    ...
                    if (intersect.isDown) {
                        ...
                        if (typeof intersect.onClick === 'function' && distance < 25) intersect.onClick()
                    }
                } else {
                    if (intersect.isDown) {
                        intersect.isDown = false

                        if (typeof intersect.onUp === 'function') intersect.onUp()
                    }
                }
            }
        }
