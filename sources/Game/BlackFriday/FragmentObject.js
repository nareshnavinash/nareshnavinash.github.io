...
export class FragmentObject {
    constructor(_position) {
        ...
        for (const _child of this.group2.children) {
            if (_child.name.startsWith('main')) this.main = _child
            else {
    ...
    update() {
        if (this.caught) this.timeMultiplier += this.game.ticker.deltaScaled * 8
        ...
        for (const _fragment of this.fragments) {
            _fragment.position.x =
                Math.sin((this.elapsedTime + _fragment.userData.timeOffset) * _fragment.userData.timeMultiplier) *
                _fragment.userData.radius *
                this.radiusMultiplier
            _fragment.position.z =
                Math.cos((this.elapsedTime + _fragment.userData.timeOffset) * _fragment.userData.timeMultiplier) *
                _fragment.userData.radius *
                this.radiusMultiplier
        }
    }
