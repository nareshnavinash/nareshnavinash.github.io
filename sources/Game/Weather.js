...
export class Weather {
    constructor() {
        ...
        // Temperature
        this.addProperty('temperature', -15, 40, () => {
        ...
        // Electric field
        this.addProperty('electricField', -1, 1, () => {
    ...
    addProperty(name, min, max, get) {
        ...
        if (this.game.debug.active) {
            ...
            this.debugPanel.addBinding(property, 'value', {
    ...
    setOverride() {
        ...
        this.override.start = (values = {}, duration = 5) => {
            for (const property of this.properties) {
                if (typeof values[property.name] !== 'undefined') property.overrideValue = values[property.name]
                else property.overrideValue = null
            }

            if (duration === 0) this.override.strength = 1
            else gsap.to(this.override, { strength: 1, duration, overwrite: true })
        }

        this.override.end = (duration = 5) => {
            if (duration === 0) this.override.strength = 0
            else gsap.to(this.override, { strength: 0, duration, overwrite: true })
        }
    }

    update() {
        for (const property of this.properties) property.binding.update()
    }
}
