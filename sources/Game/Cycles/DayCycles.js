...
const presets = {
    day: {
        ...
        lightIntensity: 1.2,
        shadowColor: new THREE.Color('#5d4fff'),
        fogColorA: new THREE.Color('#00ffff'),
        fogColorB: new THREE.Color('#9b89ff'),
        fogNearRatio: 0.315,
        fogFarRatio: 1.25
    },
    dusk: {
        revealColor: new THREE.Color('#ff86d9'),
        revealIntensity: 5.55,
        electricField: 0.25,
        temperature: 0,
        lightColor: new THREE.Color('#ff8181'),
        lightIntensity: 1.2,
        shadowColor: new THREE.Color('#4e0890'),
    ...
    night: {
        revealColor: new THREE.Color('#b678ff'),
        revealIntensity: 10,
        electricField: 1,
        temperature: -7.5,
        lightColor: new THREE.Color('#3240ff'),
        lightIntensity: 3.8,
        shadowColor: new THREE.Color('#2f00db'),
        fogColorA: new THREE.Color('#10266f'),
        fogColorB: new THREE.Color('#3a0a42'),
        fogNearRatio: -0.85,
        fogFarRatio: 1
    },
    dawn: {
        revealColor: new THREE.Color('#ff9d9d'),
        revealIntensity: 4.85,
        electricField: 0.25,
        temperature: 0,
        lightColor: new THREE.Color('#ffa882'),
        lightIntensity: 1.2,
        shadowColor: new THREE.Color('#c80050'),
        fogColorA: new THREE.Color('#f885ff'),
        fogColorB: new THREE.Color('#ff7d24'),
        fogNearRatio: 0.3,
        fogFarRatio: 1.25
    }
}
...
export class DayCycles extends Cycles {
    constructor() {
        ...
        // Always start at morning (progress ≈ 0.0) unless explicitly forced
        if (forcedProgress === null) {
            const realTimeNow = Date.now() / 1000 / this.duration
            this.startOffset = -(realTimeNow % 1)

            // Override the binding to include the morning offset
            const binding = this.newAbsoluteProgressBinding
            binding.update = () => {
                if (!binding.manual) this.newAbsoluteProgress = Date.now() / 1000 / this.duration + this.startOffset
                else this.newAbsoluteProgress = binding.manualValue
            }

            // Set initial progress to morning
            this.absoluteProgress = realTimeNow + this.startOffset
            this.newAbsoluteProgress = this.absoluteProgress
            this.progress = this.absoluteProgress % 1

            // Re-update cycle properties for morning
            this.update(true)
        }
    }
