...
export class Menu {
    ...
    setItems() {
        ...
        for (const navigationElement of navigationElements) {
            ...
            if (tabsElement) item.tabs = new Tabs(tabsElement)
            ...
            if (this.default === null) this.default = item
        }
    ...
    open(name = null) {
        ...
        if (_name === null) {
            if (this.current) _name = this.current.name
            else _name = this.default.name
        }
        ...
        // Same
        if ((this.state === Menu.OPEN || this.state === Menu.OPENING) && item === this.current) return
    ...
    close() {
        if (this.state === Menu.CLOSING || this.state === Menu.CLOSED) return
