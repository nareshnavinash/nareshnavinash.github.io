...
function populate(r) {
  ...
  if (careerGrid && r.career && r.career.positions) {
    ...
    careerGrid.innerHTML = r.career.positions.map(function (company) {
      return (company.roles || []).map(function (role) {
        ...
        var roleSections = (role.sections || []).map(function (section) {
          var titleHtml = section.title
            ? '<h4 class="timeline-section-title">' + esc(section.title) + '</h4>'
            : '';

          var pointsHtml = '<ul class="timeline-achievements">' +
        ...
        return '<div class="timeline-item reveal">' +
          ...
          '<div class="timeline-role">' + esc(role.shortRole || role.role || '') + '</div>' +
  ...
  if (pubContent && r.publications && r.publications.book) {
    ...
    pubContent.innerHTML =
      '<div class="book-card">' +
      '<div class="book-card-title">' + esc(book.title || '').replace(' ', '<br>') + '</div>' +
      '<div class="book-card-author">' + esc(book.author || '') + '</div>' +
      '</div>' +
      '<div class="pub-info">' +
      '<h3>' + esc(book.title || '') + '</h3>' +
      '<p class="pub-author">' + esc(byline) + '</p>' +
      '<p class="pub-desc">' + esc(book.description || '') + '</p>' +
      '<div class="pub-links">' +
      '<a href="' + esc(book.amazonUrl || '') + '" target="_blank" rel="noopener" class="btn-primary">' + esc(labels.publicationsAmazonCta || '') + '</a>' +
      '<a href="' + esc(book.mediumUrl || '') + '" target="_blank" rel="noopener" class="btn-secondary">' + esc(labels.publicationsMediumCta || '') + '</a>' +
      '</div></div>';
  }
  ...
  if (contactSocial && r.social) {
    contactSocial.innerHTML = primarySocialKeys.map(function (key) {
      var item = r.social[key];
      if (!item || !item.url) return '';
      var label = item.name || key;
      return '<a href="' + esc(item.url) + '" target="_blank" rel="noopener" aria-label="' + esc(label) + '">' + (socialIcons[key] || '') + '</a>';
    }).join('');
  }
  ...
  if (contactExtras && r.social) {
    contactExtras.innerHTML = extraSocialKeys.map(function (key) {
      var item = r.social[key];
      if (!item || !item.url) return '';
      return '<a href="' + esc(item.url) + '" target="_blank" rel="noopener">' + esc(item.name || key) + '</a>';
    }).join('');
  }
...
function populateSeo(r, seo) {
  ...
  var socialUrls = Object.keys(r.social || {}).map(function (key) {
...
// Export for testing
export {
  RESUME_PATHS, loadResume, populate, populateSeo, populateNavigation,
  populateHero, triggerRevealObservers, bindTimelineDetails,
  setText, setAttr, formatTemplate, esc, init
};
