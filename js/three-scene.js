...
for (let i = 0; i < STAR_COUNT; i++) {
    ...
    const c = Math.random() > 0.88 ? accentArr[Math.floor(Math.random() * accentArr.length)] : new THREE.Color(0x6666aa)
    starCol[i * 3] = c.r
    starCol[i * 3 + 1] = c.g
    starCol[i * 3 + 2] = c.b
}
...
for (let i = 0; i < NET_COUNT; i++) {
    ...
    netVel.push(
        new THREE.Vector3((Math.random() - 0.5) * 0.008, (Math.random() - 0.5) * 0.008, (Math.random() - 0.5) * 0.006)
    )
}
