function ot(t) {
    ...
    ;((t.career || []).forEach((u, y) => {
        if (u.isTail) return
        const b = ct(u.desc || ''),
            E = `${s.name || 'Naresh Sekar'} worked as ${u.role} at ${u.co} (${u.date}). ${b}`
        e.push({
            id: `career:${y}`,
            section: 'career',
...
const ht = "Incorrect 'index' type",
    dt = (t) => `Invalid value for key ${t}`,
    ft = (t) => `Pattern length exceeds max of ${t}.`,
    gt = (t) => `Missing ${t} property in key`,
    pt = (t) => `Property 'weight' in key '${t}' must be a positive integer`,
...
function Ke(t) {
    let e = null,
        ...
        i = 1,
        o = null
    if (_(t) || B(t)) ((n = t), (e = De(t)), (s = ue(t)))
    else {
        if (!$e.call(t, 'name')) throw new Error(gt('name'))
        const c = t.name
        if (((n = c), $e.call(t, 'weight') && ((i = t.weight), i <= 0))) throw new Error(pt(c))
        ...
        if (v(o))
            if (!c[r])
                ...
                if (r === c.length - 1 && (_(l) || Q(l) || lt(l) || typeof l == 'bigint'))
                    ...
                    n = !0
                    for (let f = 0, g = l.length; f < g; f += 1) i(l[f], c, r + 1, f)
...
function Ye(t, e, { getFn: s = m.getFn, fieldNormWeight: n = m.fieldNormWeight } = {}) {
    const i = new _e({
        getFn: s,
        fieldNormWeight: n
    })
...
function xt(t, { getFn: e = m.getFn, fieldNormWeight: s = m.fieldNormWeight } = {}) {
...
function Ct(
    ...
    if (e.length > N) throw new Error(ft(N))
    const l = e.length,
        f = t.length,
    ...
    for (let x = 0; x < l; x += 1) {
        let $ = 0,
            S = C
        for (; $ < S; ) (p(x, g + S) <= h ? ($ = S) : (C = S), (S = Math.floor((C - $) / 2 + $)))
        C = S
        let z = Math.max(1, g - S + 1)
        const re = c ? f : Math.min(g + S, f) + l,
            P = Array(re + 2)
        P[re + 1] = (1 << x) - 1
        for (let M = re; M >= z; M -= 1) {
            ...
            if (
                (u && (y[Y] = +!!Me),
                (P[M] = ((P[M + 1] << 1) | 1) & Me),
                x && (P[M] |= ((E[M + 1] | E[M]) << 1) | 1 | E[M + 1]),
                P[M] & rt && ((A = p(x, Y)), A <= h))
            ) {
                if (((h = A), (k = Y), k <= g)) break
                z = Math.max(1, 2 * g - k)
            }
        }
        if (p(x + 1, g) > h) break
...
const qe = {
    ...
    It = new RegExp('[' + Object.keys(qe).join('') + ']', 'g'),
    K = String.prototype.normalize
        ? (t) =>
              t
                  .normalize('NFD')
                  .replace(
...
const he = [Mt, Ge, Dt, Lt, Tt, Ft, $t, Ue],
    Fe = he.length,
    Bt = '\0',
    Rt = '|'
function Nt(t) {
...
function Ot(t, e = {}) {
    return t
        ...
        .split(Rt)
        .map((n) => {
            const i = n.replace(/\u0000/g, '|'), // eslint-disable-line no-control-regex
                o = Nt(i.trim()).filter((r) => r && !!r.trim()),
                c = []
...
class jt {
    constructor(
        ...
        ;((this.query = null),
            ...
            (e = n ? K(e) : e),
            (this.pattern = e),
            (this.query = Ot(this.pattern, this.options)))
    }
    ...
    searchIn(e) {
        ...
        for (let l = 0, f = s.length; l < f; l += 1) {
            ...
            for (let h = 0, k = g.length; h < k; h += 1) {
                const p = g[h],
                    { isMatch: u, indices: y, score: b } = p.search(e)
                if (u) {
                    ;((c += 1), (a += b))
...
function Qe(t, e, { auto: s = !0 } = {}) {
    const n = (i) => {
        ...
        const o = Object.keys(i),
            c = zt(i)
        if (!c && o.length > 1 && !ge(i)) return n(Te(i))
        if (Ht(i)) {
            const a = c ? i[fe.PATH] : o[0],
                d = c ? i[fe.PATTERN] : i[a]
            if (!_(d)) throw new Error(dt(a))
            const l = {
                keyId: ue(a),
                pattern: d
            }
...
function pe(t, { ignoreFieldNorm: e = m.ignoreFieldNorm }) {
    let s = 1
    return (
...
function Wt(t, { ignoreFieldNorm: e = m.ignoreFieldNorm }) {
    t.forEach((s) => {
...
function Ut(t, e, { includeMatches: s = m.includeMatches, includeScore: n = m.includeScore } = {}) {
    const i = []
    return (
        s && i.push(Yt),
        n && i.push(qt),
...
const Gt = /\b\w+\b/g
function me({ isCaseSensitive: t = !1, ignoreDiacritics: e = !1 } = {}) {
    return {
        tokenize(s) {
    ...
    for (const r of t) {
        ...
        if (l)
            for (let f = 0; f < e; f++) {
                const g = l[f]
                if (g)
                    if (Array.isArray(g)) for (const h of g) c(h.v, a, f, h.i ?? -1)
                    else c(g.v, a, f, -1)
            }
    }
    return {
...
function Vt(t, e, s, n) {
    ...
    if (c)
        for (let a = 0; a < s; a++) {
            const d = c[a]
            if (d)
                if (Array.isArray(d)) for (const l of d) r(l.v, a, l.i ?? -1)
                else r(d.v, a, -1)
        }
}
function Be(t, e) {
    for (const [s, n] of t.terms) {
        const i = n.filter((c) => c.docIdx !== e),
            o = n.length - i.length
        o > 0 &&
            ((t.fieldCount -= o),
...
class L {
    ...
    setCollection(e, s) {
        if (((this._docs = e), s && !(s instanceof _e))) throw new Error(ht)
        if (
            ((this._myIndex =
                s ||
                Ye(this.options.keys, this._docs, {
    ...
    remove(e = () => !1) {
        ...
        if (n.length) {
            if (this._invertedIndex) for (const i of n) Be(this._invertedIndex, i)
    ...
    search(e, s) {
        const { limit: n = -1 } = s || {},
            { includeMatches: i, includeScore: o, shouldSort: c, sortFn: r, ignoreFieldNorm: a } = this.options
        if (_(e) && !e.trim()) {
    ...
    _searchObjectList(e, { heap: s, ignoreFieldNorm: n } = {}) {
        const i = this._getSearcher(e),
            { keys: o, records: c } = this._myIndex,
            r = s ? null : []
        return (
            c.forEach(({ $: a, i: d }) => {
                if (!v(a)) return
    ...
    _findMatches({ key: e, value: s, searcher: n }) {
        ...
        const i = []
        if (B(s))
            s.forEach(({ v: o, i: c, n: r }) => {
                if (!v(o)) return
                const { isMatch: a, score: d, indices: l, hasInverse: f } = n.searchIn(o)
                a &&
                    i.push({
                        score: d,
                        key: e,
            ...
            const { v: o, n: c } = s,
                { isMatch: r, score: a, indices: d, hasInverse: l } = n.searchIn(o)
            r &&
                i.push({
                    score: a,
                    key: e,
...
class Jt {
    ...
    searchIn(e) {
        ...
        for (let a = 0; a < this.termSearchers.length; a++) {
            const d = this.termSearchers[a].searchIn(e),
                l = this.idfWeights[a]
...
function ns(t, e, s) {
    ...
    if (!n) return null
    if (ke.keywords.some((r) => n.includes(r))) return { intent: ke, confidence: 1 }
    for (const r of Ve)
        if (
            r.keywords.some((d) => {
    ...
    if (c.length) {
        const r = c[0],
            ...
            d = r.item.intentId,
            l = Je.find((f) => f.id === d)
        if (l && a > 0.7) return l.type === 'navigate' ? { intent: l, confidence: a } : { intent: l, confidence: a }
        if (l && a > 0.4) return { intent: l, confidence: a }
    }
    return {
...
const Xe = 'https://naresh-ai-proxy.nareshnavinash.workers.dev',
    os = 'openai/gpt-oss-20b',
    cs = 'gpt-oss-20b'
function as() {
    return Xe.length > 0
}
...
const us = /* @__PURE__ */ Object.freeze(
    ...
    Ze = 'https://naresh-ai-proxy.nareshnavinash.workers.dev',
    hs = 'gemini-flash'
function ds() {
    return Ze.length > 0
}
...
const gs = /* @__PURE__ */ Object.freeze(
        /* @__PURE__ */ Object.defineProperty(
            {
                __proto__: null,
                generate: fs,
...
async function ws(t, e) {
    if (!ys().allowed) throw new Error('DAILY_LIMIT')
    const n = Date.now()
...
function Es(t, e) {
    ...
    const s = ce(t)
    for (const c of e) if (ce(c.q) === s) return c
    ...
    for (const c of e) {
        const r = ce(c.q)
        let a = 0
        for (const l of n) r.includes(l) && a++
...
async function As(t, e, s) {
    const n = e.search(t, 5),
        i =
            n.length >= 3
                ? n
                : n.length
                  ? [...n, ...Oe(s).filter((l) => !n.some((f) => f.id === l.id))].slice(0, 5)
                  : Oe(s),
        o = i.map((l) => ({
            id: l.id,
            section: l.section,
    ...
    try {
        ...
        return (
            f === 'DAILY_LIMIT'
                ? (g = `You've reached the daily limit (${be()}/${Ee()}). Come back tomorrow! Here's what I found locally:`)
                : f === 'RATE_LIMITED'
                  ? (g = "naresh.ai is popular today — I've hit the rate limit. Here's what I found locally:")
                  : f === 'NO_API_KEY' && (g = "AI answers aren't configured. Here's what I found in the resume:"),
            {
                type: 'fallback',
                text: g,
                sources: o,
    ...
    ;((t.innerHTML = ''),
        e.forEach((n) => {
            if (n.role === 'a') {
                ...
                ;((c.className = `msg msg--a${n.variant === 'error' ? ' msg--error' : ''}`),
                    c.append(i, o),
                    t.append(c))
            } else if (n.role === 'u') {
                const i = document.createElement('div')
                ...
                ;((i.className = 'msg__sources'),
                    (n.items || []).forEach((o) => {
                        ...
                        ;((c.className = 'msg__src'),
                            (c.textContent = o.label || o.section),
                            c.addEventListener('click', () => xs(o, s)),
                            i.append(c))
                    }),
                    t.append(i))
            }
...
function W(t) {
    w?.statusEl &&
        ((w.statusEl.dataset.state = t),
        (w.statusEl.textContent = t === 'ready' ? 'ready' : t === 'thinking' ? 'thinking...' : t))
}
function Pe() {
...
function Ds() {
    ...
    ;(document.body.insertAdjacentHTML(
        ...
        (Ae = D.querySelector('.cmdk__rate')),
        D.querySelector('.cmdk__backdrop').addEventListener('click', se))
    let e = null
    ;(O.addEventListener('input', () => {
        ...
        O.addEventListener('keydown', (s) => {
            ...
            s.key === 'ArrowDown'
                ? (s.preventDefault(), (I = (I + 1) % Math.max(n, 1)), ne())
                : s.key === 'ArrowUp'
                  ? (s.preventDefault(), (I = (I - 1 + Math.max(n, 1)) % Math.max(n, 1)), ne())
                  : s.key === 'Enter'
                    ? (s.preventDefault(), Fs())
                    : s.key === 'Escape' && (s.preventDefault(), se())
        }))
}
function Ls(t) {
    ;((j = t),
        document.addEventListener('keydown', (s) => {
            ;(s.metaKey || s.ctrlKey) &&
                s.key === 'k' &&
                (s.preventDefault(), D?.classList.contains('open') ? se() : ze())
        }))
...
function ze() {
    D &&
        ...
        (I = 0),
        nt(''),
        Ae && X?.getRemaining && (Ae.textContent = `${X.getRemaining()}/${X.getMax()} AI queries today`),
...
function nt(t) {
    ...
    let e
    t
        ? ((e = Ie.search(t, { limit: 8 }).map((o) => o.item)),
          t.length >= 3 &&
              e.push({
                  category: 'Ask AI',
                  icon: '✦',
    ...
    ;(s.forEach((i, o) => {
        const c = document.createElement('div')
        ;((c.className = 'cmdk__group-label'),
            (c.textContent = o),
            T.append(c),
            i.forEach((r) => {
                const a = document.createElement('div')
                ;((a.className = 'cmdk__item'),
                ...
                <div class="cmdk__item-text">
                    ...
                    a.addEventListener('click', () => it(r.action)),
...
function Fs() {
    ...
    let s
    ;(e
        ? ((s = Ie.search(e, { limit: 8 }).map((n) => n.item)),
          e.length >= 3 && s.push({ action: { type: 'ask', query: e } }))
        : (s = Se.filter((n) => n.category === 'Sections' || n.category === 'Career')),
        s[I] && it(s[I].action))
}
function it(t) {
    if (!(!t || !j))
...
function Ns() {}
export { Ns as destroy, Rs as init }
