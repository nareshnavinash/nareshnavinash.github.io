...
;(() => {
    ...
    function mountHeroFlyIn() {
        ...
        chars.forEach((c, i) => {
            c.style.transitionDelay = `${Math.min(i * 22, 1500)}ms`
        })
    ...
    function mountEndCard() {
        ...
        const end = el(
            'div',
            { class: 'footer-end', id: 'contact' },
            el('h2', { class: 'footer-end__name' }, 'Naresh Sekar'),
            el(
                'div',
                { class: 'footer-end__run' },
                el('span', { class: 'dot' }),
                el('span', { html: `runtime · <span id="runtime">0:00</span>` })
            ),
            el(
                'div',
                { class: 'footer-end__socials' },
                el('a', { href: 'mailto:nareshnavinash@gmail.com' }, 'email'),
                el(
                    'a',
                    { href: 'https://www.linkedin.com/in/nareshnavinash/', target: '_blank', rel: 'noreferrer' },
                    'linkedin'
                ),
                el(
                    'button',
                    { id: 'only-fans-btn', class: 'of-btn', type: 'button', 'aria-label': 'Only Fans' },
                    'Only Fans'
                ),
